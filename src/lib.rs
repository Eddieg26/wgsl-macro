...
impl<'a> ShaderProcessor<'a> {
    ...
    fn process_tokens(
        ...
        while let Some(token) = tokens.next() {
            match token {
                ...
                Token::ElseIf(line) => return Err(ShaderProcessorError::UnmatchedElse { line }),
                Token::ElseIfDef(line) => return Err(ShaderProcessorError::UnmatchedElse { line }),
            }
        }
    ...
    fn process_branch_blocks(
        ...
        for block in blocks {
            ...
            match result {
                Ok(false) => continue,
                Ok(true) => return self.process_tokens(&mut block.tokens.into_iter(), constants),
