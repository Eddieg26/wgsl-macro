  use std::{
    ...
    impl ShaderConstant {
        ...
        fn greater_than_or_equal<'a>(
            &self,
            value: &'a str,
        ) -> Result<bool, ShaderProcessorError<'a>> {
            match self {
                ShaderConstant::I32(v) => value
                    .parse::<i32>()
                    .ok()
    ...
    pub struct BranchBlock<'a> {
        line: &'a str,
        eval: Option<
            for<'b> fn(&'b str, &'b ShaderConstants) -> Result<bool, ShaderProcessorError<'b>>,
        >,
        tokens: Vec<Token<'a>>,
    }
    ...
    impl<'a> ShaderProcessor<'a> {
        ...
        pub async fn get_imports<F, E, Ctx>(
            src: &'a str,
            ctx: &'a mut Ctx,
            mut resolve: impl FnMut(&str, &mut Ctx) -> F,
        ...
        fn eval_condition<'b>(
            ...
            if let Some(pos) = condition.find("==") {
                let (value, constant) =
                    parse(pos).ok_or(ShaderProcessorError::InvalidCondition {
